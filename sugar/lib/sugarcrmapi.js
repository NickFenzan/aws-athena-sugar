...
var path_join = function path_join() {
 ...
 * @param {string} secret - Client API secret
...
function Connection(key, secret) {
	...
	var _secret = ''
	var _token = ''
	var _hostname = 'sugar.millervein.com'
  var _version = '/sugar/rest/v10'
	...
	var authenticate = function() {
		...
		// Set up the request
		var verb = 'POST'
		var path = path_join(_version, '/oauth2/token')
		var params = {
      "grant_type":"password",
      "client_id":"sugar",
      "client_secret": "",
    ...
    var paramString = JSON.stringify(params);
      ...
      'content-length': paramString.length
		...
		call(verb, path, paramString, headers, false)
	...
	var authorized_call = function(verb, path, body, headers) {
		...
		var new_path = path_join(_version, path)
			...
			.on('401', function() {
				authenticate()
					.on('ready', function() {
						...
						new_headers['oauth-token'] = _token
